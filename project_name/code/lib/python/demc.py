...
#Block DEMC
#NEED TO BLOCK THE MODEL, NOT EVENT!!!!
...
def calcChi2(nchains, functype, myfuncs, pedit, nextp, iortholist, funcx, cummodels, numparams, j, isrednoise, wavelet, noisefunc, systematics, chains=None):
    ...
    for n in chains:
        if isrednoise == False:
            #FINDME: C code fails with K2-21 K2 light curve in /home/kevin/Documents/UChicago/analyses/k2-21/jointFit/2016-04-16-NewK2LC, not sure why.
...
def calcModel_old(fit, functype, myfuncs, pedit, nextp, iortholist, funcx, k, nump1, nump2, n):
    '''
    Compute model light curve by combining model components.
    '''
    ymodel    = np.ones(fit.nobj)
    noisepars = None
    if   functype == 'ortho':
        pedit[iortholist] = myfuncs(pedit[iortholist], funcx, fit.etc[k])
    elif functype == 'ipmap':
        fit.etc[k]  = ymodel
        ymodel     *= myfuncs(pedit[nump1:nump2], funcx, fit.etc[k])
    elif functype == 'posoffset':
        # Record change in Position 0 => cannot orthogonalize position parameters
        ymodel     *= myfuncs(nextp[nump1:nump2], funcx, fit.etc[k])
    elif hasattr(fit, 'timebins') and (functype == 'ecl/tr' 
                                   or  functype == 'ramp' 
                                   or  functype == 'sinusoidal'):
        # Average over high-resolution model
        hiresmodel = myfuncs(pedit[nump1:nump2], funcx, fit.etc[k])
        if len(fit.timebins) == fit.nobj:
            for tb in range(len(fit.timebins)):
                ymodel[tb] *= np.mean(hiresmodel[fit.timebins[tb]])
        else:
            for tb in range(len(fit.timebinsuc)):
                ymodel[tb] *= np.mean(hiresmodel[fit.timebinsuc[tb]])
    elif functype == 'noise':
        noisepars    = pedit[n,nump1:nump2]
    else:
        ymodel    *= myfuncs(pedit[nump1:nump2], funcx, fit.etc[k])
    
    return [ymodel, noisepars, n]
...
def writeModel(arg):
    ...
    Write models after multiprocessing.
    '''
    ymodel, noisepar, n = arg
    # Assign to arrays
    ymodels[n]  *= ymodel
    noisepars[n] = noisepar
    return

def demc_old(y, pars, pmin, pmax, stepsize, numit, sigma, numparams, cummodels, functype, myfuncs, funcx, iortholist, fit, gamma=None, isGR=True, ncpu=1):
    #data, t, sigma, pars, numit=1000, gamma=None):
    '''
    This function uses a differential evolution Markov chain to assess uncertainties.
    ...
    for j in range(numevents):
        ...
        for i in range(cummodels[j],cummodels[j+1]):
            ...
            if ncpu == 1:
                # Only 1 CPU
                for n in range(nchains):
                    writeModel(calcModel_old(fit[j], functype[i], myfuncs[i], pedit[n], nextp[n], iortholist[j], funcx[i], k, numparams[i], numparams[i+1], n))
            else:
                # Multiple CPUs
                pool = mp.Pool(ncpu)
                for n in range(nchains):
                    res = pool.apply_async(calcModel_old, args=(fit[j], functype[i], myfuncs[i], pedit[n], nextp[n], iortholist[j], funcx[i], k, numparams[i], numparams[i+1], n), callback=writeModel)
    ...
    for m in range(numit):
        ...
        for j in range(numevents):
            ...
            for i in range(cummodels[j],cummodels[j+1]):
                ...
                for n in range(nchains):
                    if   functype[i] == 'ortho':
                        ...
                        noisepars    = pedit[n,numparams[i]:numparams[i+1]]
                    else:
                        ymodels[n] *= myfuncs[i](pedit[n,numparams[i]:numparams[i+1]], funcx[i], fit[j].etc[k])
                '''
                k += 1
            ...
            for n in range(nchains):
                if isrednoise == False:
                    nextchisq[n]  += mc.chisq(ymodels[n], y[j], sigma[j])
                else:
                    nextchisq[n]  += noisefunc(noisepars[n], ymodels[n]-y[j], wavelet)
                # Apply prior, if one exists
                if len(fit[j].ipriors) > 0:
                    pbar   = fit[j].priorvals[:,0]  #prior mean
                    psigma = np.zeros(len(pbar))    #prior standard deviation
                    ...
                    for i in range(len(fit[j].ipriors)):
                        if nextp[n,fit[j].ipriors[i]] < pbar[i]:
                            ...
                            psigma[i] = fit[j].priorvals[i,2]
                        nextchisq[n] += ((nextp[n,fit[j].ipriors[i]] - pbar[i])/psigma[i])**2
