...
def seramp(rampparams, x, etc = []):
    ...
    return ne.evaluate('goal + pm*exp(-r0*x + r1)')
...
def selramp(rampparams, x, etc = []):
  ...
  2010-07-05    Kevin Stevenson, UCF
    ...
    return ne.evaluate('goal + pm*exp(-r0*x + r1) + r2*(x-x0)')
...
def seqramp(rampparams, x, etc = []):
  ...
  2010-07-05    Kevin Stevenson, UCF
    ...
    return ne.evaluate('goal + pm*exp(-r0*x + r1) + r2*(x-x0) + r3*(x-x0)**2')
...
def se2ramp(rampparams, x, etc = []):
  ...
  2011-07-11    Kevin Stevenson, UCF
    ...
    return ne.evaluate('goal + pm0*exp(-r0*x+r1) + pm1*exp(-r4*x+r5)')
...
def reramp2(rampparams, x, etc = []):
  ...
  2008-06-24    Kevin Stevenson, UCF
   ...
   return ne.evaluate('goal*(1 - exp(-a*x + b))')
...
def logramp(rampparams, x, etc = []):
  ...
  2008-06-26    Kevin Stevenson, UCF
    ...
    output[np.where(x > x0)] = ne.evaluate('a*log(xnew-x0)**4 + b*log(xnew-x0)**3 + c*log(xnew-x0)**2 + d*log(xnew-x0) + e')
...
def rotation(rotparams, x, etc = []):
    ...
    return ne.evaluate('1 + a*airmass + b*cos(pi/180*(rotation+offset))')
