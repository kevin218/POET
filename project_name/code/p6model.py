# $Author: kevin $
# $Revision: 667 $
# $Date: 2012-05-02 11:35:21 -0400 (Wed, 02 May 2012) $
# $HeadURL: file:///home/esp01/svn/code/python/pipeline/trunk/p6model.py $
# $Id: p6model.py 667 2012-05-02 15:35:21Z kevin $
...
"""
 MODIFICATION HISTORY:
    ...
    Non-fixed priors in least-squares       2014-01-08
...
#reload(mc), change to import lib.reload
import mcmc
#reload(mcmc)
import demc
#reload(demc)
...
def rundmc(event, num=0, printout=sys.stdout, isinteractive=True):
    ...
    #When normflux=False: flux is not normalized w.r.t. each position's median value.
    ...
    if hasattr(event[0].params, 'night'):
        ...
        nights      = np.zeros(numevents, dtype=int)
    if hasattr(event[0].params, 'nbins'):
        pass
    else:
        for j in range(numevents):
            event[j].params.nbins = int(np.sum(event[j].good))
    ...
    for j in range(numevents):
        ...
        #print('bjdtbduc:')
        #print(fit[j].bjdtdbuc)
        #fit[j].airmassuc  = event[j].airmass[np.where(event[j].good == 1)]
        #fit[j].rotationuc = event[j].rotation[np.where(event[j].good == 1)]
        fit[j].phaseuc      = event[j].phase [np.where(event[j].good == 1)]
        fit[j].framenumuc   = event[j].framenum[np.where(event[j].good == 1)]
        fit[j].batchnumuc   = event[j].batchnum[np.where(event[j].good == 1)]
        fit[j].orbitnumuc   = event[j].orbitnum[np.where(event[j].good == 1)]
        fit[j].posuc        = event[j].pos   [np.where(event[j].good == 1)]
        try:
            fit[j].yuc      = np.mean(event[j].drift2D[:,:,0],axis=1)[np.where(event[j].good == 1)]
            fit[j].xuc      = np.mean(event[j].drift2D[:,:,1],axis=1)[np.where(event[j].good == 1)]
        except:
            print("No 2D drift information. Setting positions to zero.")
            fit[j].yuc      = np.zeros(len(fit[j].fluxuc))
            fit[j].xuc      = np.zeros(len(fit[j].fluxuc))
        '''
    ...
    for j in range(numevents):
        ...
        '''
        #CHECK FOR NOISE PARAMETERS, WHICH ARE FIXED DURING LEAST-SQUARES FIT
        for i in range(cummodels[j],cummodels[j+1]):
            if functype[i] == 'noise':
                fit[j].isprior[numparams[i]:numparams[i+1]] = 1
        '''
        ...
        isclipmask      = np.where(fit[j].clipmask)[0]
        fit[j].ipmask   = np.copy(fit[j].ipmaskuc[isclipmask])
        fit[j].position = np.array([fit[j].yuc[isclipmask], 
                           fit[j].xuc[isclipmask],
                           np.zeros(len(isclipmask))])
        fit[j].positionuc   = np.array([fit[j].yuc, 
                                        fit[j].xuc,
                                        np.zeros(len(fit[j].yuc))])
        '''
        fit[j].position = np.array([fit[j].y[fit[j].isclipmask], 
                                    fit[j].x[fit[j].isclipmask], 
                                    fit[j].quadrant[fit[j].isclipmask]])
        '''
        fit[j].nobj     = fit[j].ipmask.size
        ...
        fit[j].isclipmask = np.where(fit[j].clipmask)
        #print('isclipmask:')
        #print(fit[j].isclipmask)
        ...
        #fit[j].airmass    = fit[j].airmassuc[fit[j].isclipmask]
        #fit[j].rotation   = fit[j].rotationuc[fit[j].isclipmask]
        ...
        fit[j].framenum   = np.copy(fit[j].framenumuc[fit[j].isclipmask])
        fit[j].batchnum   = np.copy(fit[j].batchnumuc[fit[j].isclipmask])
        fit[j].orbitnum   = np.copy(fit[j].orbitnumuc[fit[j].isclipmask])
    ...
    for j in range(numevents):
        ...
        #Increase time resolution for longer exposures
        if hasattr(event[j].params, 'expand') and event[j].params.expand > 1:
            itimeuc = np.arange(0, fit[j].nobjuc, 1./event[j].params.expand) - 0.5
            fit[j].hirestimeuc = np.interp(itimeuc, range(fit[j].nobjuc), fit[j].timeunituc)
            fit[j].hiresframenum = np.interp(itimeuc, range(fit[j].nobjuc), fit[j].framenum)
            fit[j].timebinsuc = []
            for i in range(len(fit[j].timeunituc)):
                fit[j].timebinsuc.append(np.where(np.abs(fit[j].hirestimeuc - fit[j].timeunituc[i]) < event[j].exptime[i]/86400./2.)[0])
            exptime         = event[j].exptime[fit[j].isclipmask]
            fit[j].timebins = []
            for i in range(len(fit[j].timeunit)):
                fit[j].timebins.append(np.where(np.abs(fit[j].hirestimeuc - fit[j].timeunit[i]) < exptime[i]/86400./2.)[0])
        ...
        for i in range(cummodels[j],cummodels[j+1]):
            ...
            if   functype[i] == 'ecl/tr':
                ...
                if hasattr(event[j].params, 'expand') and event[j].params.expand > 1:
                    funcx.  append(fit[j].hirestimeuc)
                    funcxuc.append(fit[j].hirestimeuc)
                    fit[j].etc.append([fit[j].hiresframenum])
                else:
                    funcx.  append(fit[j].timeunit)
                    funcxuc.append(fit[j].timeunituc)
                    fit[j].etc.append([fit[j].framenum])
                ...
                if hasattr(event[j].params, 'expand') and event[j].params.expand > 1:
                    ...
                    funcxuc.append(fit[j].timeunituc)
                fit[j].etc.append([])
            elif functype[i] == 'instrument':
                funcx.  append([fit[j].airmass,   fit[j].rotation])
                funcxuc.append([fit[j].airmassuc, fit[j].rotationuc])
                fit[j].etc.append([])
    ...
    for j in range(numevents):
        ...
        fit[j].isgood    = np.where(event[j].good)
    for k in np.unique(nights):
        tonight   = np.where(nights == k)[0]
        whitelcfile = "whiteLC-Night"+str(k)+".npz"
        if os.path.isfile(whitelcfile):
            foo = np.load(whitelcfile)
            print("Using saved white light curve file for night " + str(k) + ".")
            for night in tonight:
                fit[night].whiteparams  = event[tonight[0]].params.whiteparams
                fit[night].iswhitelc    = event[tonight[0]].params.iswhitelc
                event[night].whitelc    = foo['evwhitelc']
                fit[night].whitelcuc    = event[night].whitelc[fit[night].isgood]
                fit[night].whitelc      = fit[night].whitelcuc[fit[night].isclipmask]
                #fit[night].whitelcuc    = foo['fitwhitelcuc']
                #fit[night].whitelc      = foo['fitwhitelc']
            # Construct reference star spectroscopic light curves
            # Fot HST, this will default to no reference star.
            ...
            # Fot HST, this will default to no reference star.
            nasc.refStarSpecLC2(event, fit, tonight)
            # Save white light curves
            j = tonight[0]
            if hasattr(event[tonight[0]].params, 'iswhitelc') and event[tonight[0]].params.iswhitelc != False:
                np.savez(event[j].modeldir + "/" + whitelcfile, evwhitelc=event[j].whitelc)
    ...
    if leastsq == True:
        def modelfunc(freepars, params, sigma):
            ...
            for j in range(numevents):
                ...
                fit0 *= fit[j].bestsys
                residuals = np.concatenate((residuals,(flux[j] - fit0)/sigma[j]),axis=0)
        ...
        if isnoise == True:
            ...
            #output, nfeval, rc = op.fmin_tnc(fminfunc, x0=params[inonprior], approx_grad=True, bounds=bounds, scale=stepsize[inonprior].tolist())
            #print(output)
            output = op.fmin_cobyla(fminfunc, params[inonprior], constraint, rhobeg=stepsize[inonprior])
            ...
            output, cov_x, infodict, mesg, err = op.leastsq(modelfunc, params[ifreepars], args=(params, sigma), \
                 factor=100, ftol=1e-8, xtol=1e-8, gtol=1e-8, diag=1./stepsize[ifreepars], full_output=True, maxfev=2000)
            print(mesg)
            print(output)
        '''
        if (err >= 1) and (err <= 4):
        ...
        '''
    #**************************
    #FINDME: setup is finished.  maybe a good point to split up p6model
    ...
    for j in range(numevents):
        ...
        for i in range(cummodels[j],cummodels[j+1]):
            if   functype[i] == 'ortho':
                ...
                hiresmodel = myfuncs[i](params[iparams[i]], funcx[i], fit[j].etc[k])
                for n in range(len(fit[j].timebins)):
                    fit[j].fit0[n] *= np.mean(hiresmodel[fit[j].timebins[n]])
            else:
    ...
    print('Number of iterations: ' + str(nchains*numit[0]), file=printout)
    ...
    # Construct best non-analytic systematics models
    for k in np.unique(nights):
        tonight   = np.where(nights == k)[0]
        nasc.nasmodel(event, fit, bestp, iparams, cummodels, functype, myfuncs, funcxuc, tonight)
        ...
        for i in range(cummodels[j],cummodels[j+1]):
            if   functype[i] == 'ortho':
                ...
                fit[j].bestfituc  *= fit[j].ballardipuc
                                              ...
                                              or  functype[i] == 'sinusoidal'):
                # Average over high-resolution model
                hiresmodel = myfuncs[i](bestp[iparams[i]], funcx[i], fit[j].etc[k])
                for n in range(len(fit[j].timebins)):
                    fit[j].bestfit[n]   *= np.mean(hiresmodel[fit[j].timebins[n]])
                hiresmodel = myfuncs[i](bestp[iparams[i]], funcxuc[i], fit[j].etc[k])
                for n in range(len(fit[j].timebinsuc)):
                    fit[j].bestfituc[n] *= np.mean(hiresmodel[fit[j].timebinsuc[n]])
            else:
            ...
            k += 1
        fit[j].bestfit   *= fit[j].bestsys
        fit[j].bestfituc *= fit[j].bestsysuc
    
    for j in range(numevents):
        fit[j].redchisq = sum((fit[j].bestfit - fit[j].flux)**2 / fit[j].sigma**2) / \
                              (fit[j].nobj - fit[j].numfreepars)
        print("Reduced Chi^2: " + str(fit[j].redchisq))
    ...
    if leastsq == True:
        if isnoise == True:
            ...
            bounds = np.concatenate((pmin[:,np.newaxis][inonprior], pmax[:,np.newaxis][inonprior]), axis=1)
            #output, nfeval, rc = op.fmin_tnc(fminfunc, x0=bestp[inonprior], approx_grad=True, bounds=bounds, scale=stepsize[inonprior].tolist())
            #print(output)
            output = op.fmin_cobyla(fminfunc, params[inonprior], constraint, rhobeg=stepsize[inonprior])
            ...
            output, err = op.leastsq(modelfunc, bestp[ifreepars], args=(bestp, newsigma), factor=100, \
                                         ftol=1e-16, xtol=1e-16, gtol=1e-16, diag=1./stepsize[ifreepars], maxfev=2000)
            if (err >= 1) and (err <= 4):
                print("Fit converged without error.")
            else:
    ...
    print('Max number of iterations: ' + str(nchains*numit[1]), file=printout)
    if nchains == 1 and isresidperm == False:
        ...
        #               newsigma, numparams, cummodels, functype, myfuncs, funcx, \
        #               iortholist, nights, fit, isGR=True, gamma=gamma)
    print('MARK: ' + time.ctime() + ' : End full mcmc model', file=printout)
    print('Completed ' + str(numit[1]) + ' iterations.', file=printout)
    ...
    for j in range(numevents):
        if event[j].params.timeunit == 'orbits':
            ...
            # Construct new independent variable for displaying transit model
            fit[j].modelfuncx = np.linspace(fit[j].phase[ 0]-event[j].params.tuoffset, \
                                            fit[j].phase[-1]-event[j].params.tuoffset, \
                                            event[j].params.nbins*20)
        elif event[j].params.timeunit == 'days-utc':
            ...
            fit[j].modelfuncx = np.linspace(fit[j].bjdutc[ 0]-event[j].params.tuoffset, \
                                            fit[j].bjdutc[-1]-event[j].params.tuoffset, \
                                            event[j].params.nbins*20)
        elif event[j].params.timeunit == 'days-tdb':
            ...
            fit[j].modelfuncx = np.linspace(fit[j].bjdtdb[ 0]-event[j].params.tuoffset, \
                                            fit[j].bjdtdb[-1]-event[j].params.tuoffset, \
                                            event[j].params.nbins*20)
        else:
            fit[j].abscissa   = fit[j].binbjdutc   - event[j].params.tuoffset
            fit[j].abscissauc = fit[j].binbjdutcuc - event[j].params.tuoffset
            ...
            fit[j].modelfuncx = np.linspace(fit[j].bjdutc[ 0]-event[j].params.tuoffset, \
                                            fit[j].bjdutc[-1]-event[j].params.tuoffset, \
                                            event[j].params.nbins*20)
        fit[j].nmodelpts  = len(fit[j].modelfuncx)
        if hasattr(fit[j], 'wherepos'):
            fit[j].modelwherepos = []
            for i in range(len(fit[j].wherepos)):
                postimes = fit[j].timeunit[fit[j].wherepos[i]]
                imodelpostimes = []
                for n in range(len(postimes)):
                    imodelpostimes = np.concatenate((imodelpostimes,[np.argsort(np.abs(fit[j].modelfuncx-postimes[n]))[0]]),axis=0)
                fit[j].modelwherepos.append(imodelpostimes.astype(int))
            fit[j].posmodelfuncx = [fit[j].nmodelpts, fit[j].modelwherepos]
        if hasattr(event[j], 'drift2D'):
            yspline = spi.UnivariateSpline(fit[j].abscissa,fit[j].position[0],s=0,k=1)
            xspline = spi.UnivariateSpline(fit[j].abscissa,fit[j].position[1],s=0,k=1)
            qspline = spi.UnivariateSpline(fit[j].abscissa,fit[j].position[2],s=0,k=1)
            fit[j].ipmodelfuncx = np.array([yspline(fit[j].modelfuncx),
                                            xspline(fit[j].modelfuncx),
                                            qspline(fit[j].modelfuncx)])
    
    # Construct best non-analytic systematics models
    for k in np.unique(nights):
        tonight   = np.where(nights == k)[0]
        nasc.nasmodel(event, fit, bestp, iparams, cummodels, functype, myfuncs, funcxuc, tonight)
        # Construct systematics model for HST plots
        #systematics2 = nasc.whiteLCmodel2(event, fit, bestp, iparams, numevents, cummodels, functype, myfuncs, funcxuc)
        nasc.whiteLCmodel3(event, fit, bestp, iparams, numevents, cummodels, functype, myfuncs, funcxuc, tonight)
    ...
    for j in range(numevents):
        ...
        fit[j].bestp      = bestp    [numparams[cummodels[j]]:numparams[cummodels[j+1]]]
        fit[j].bestfit    = np.ones(fit[j].nobj)
        fit[j].modelfit   = np.ones(fit[j].nmodelpts)
        k = 0
        for i in range(cummodels[j],cummodels[j+1]):
            if   functype[i] == 'ortho':
                pass
                ...
                for n in range(len(fit[j].timebins)):
                    fit[j].bestfit[n] *= np.mean(hiresmodel[fit[j].timebins[n]])
                fit[j].modelfit *= myfuncs[i](bestp[iparams[i]], fit[j].modelfuncx, fit[j].etc[k])
            elif functype[i] == 'posoffset':
                fit[j].bestfit  *= myfuncs[i](bestp[iparams[i]], funcx[i], fit[j].etc[k])
                fit[j].modelfit *= myfuncs[i](bestp[iparams[i]], fit[j].posmodelfuncx, fit[j].etc[k])
            elif functype[i] == 'ippoly':
                fit[j].bestfit  *= myfuncs[i](bestp[iparams[i]], funcx[i], fit[j].etc[k])
                fit[j].modelfit *= myfuncs[i](bestp[iparams[i]], fit[j].ipmodelfuncx, fit[j].etc[k])
            else:
                fit[j].bestfit  *= myfuncs[i](bestp[iparams[i]], funcx[i], fit[j].etc[k])
                fit[j].modelfit *= myfuncs[i](bestp[iparams[i]], fit[j].modelfuncx, fit[j].etc[k])
            ...
            k += 1
        fit[j].bestfit   *= fit[j].bestsys
        #print(fit[j].systematics2)
        #Try to multiply 'modelfit' by systematics
        #Error will only affect binned plot (fig6001)
        try:
            fit[j].modelfit  *= fit[j].systematics2
        except ValueError as detail:
            print(detail)
            print(detail, file=printout)
        ...
        for i in range(cummodels[j],cummodels[j+1]):
            if   functype[i] == 'ecl/tr':
                if hasattr(fit[j], 'timebins'):
                    hiresmodel = myfuncs[i](bestp[iparams[i]], funcx[i], fit[j].etc[k])
                    for n in range(len(fit[j].timebins)):
                        fit[j].bestecl[n]   *= np.mean(hiresmodel[fit[j].timebins[n]])
                    hiresmodel = myfuncs[i](bestp[iparams[i]], funcxuc[i], fit[j].etc[k])
                    for n in range(len(fit[j].timebinsuc)):
                        fit[j].bestecluc[n] *= np.mean(hiresmodel[fit[j].timebinsuc[n]])
                else:
                ...
                if hasattr(fit[j], 'timebins'):
                    hiresmodel = myfuncs[i](bestp[iparams[i]], funcx[i], fit[j].etc[k])
                    for n in range(len(fit[j].timebins)):
                        fit[j].bestramp[n]   *= np.mean(hiresmodel[fit[j].timebins[n]])
                    hiresmodel = myfuncs[i](bestp[iparams[i]], funcxuc[i], fit[j].etc[k])
                    for n in range(len(fit[j].timebinsuc)):
                        fit[j].bestrampuc[n] *= np.mean(hiresmodel[fit[j].timebinsuc[n]])
                else:
                ...
                if hasattr(fit[j], 'timebins'):
                    hiresmodel = myfuncs[i](bestp[iparams[i]], funcx[i], fit[j].etc[k])
                    for n in range(len(fit[j].timebins)):
                        fit[j].bestsin[n]   *= np.mean(hiresmodel[fit[j].timebins[n]])
                    hiresmodel = myfuncs[i](bestp[iparams[i]], funcxuc[i], fit[j].etc[k])
                    for n in range(len(fit[j].timebinsuc)):
                        fit[j].bestsinuc[n] *= np.mean(hiresmodel[fit[j].timebinsuc[n]])
                else:
    ...
    for j in range(numevents):
        ...
        maxstepsize         = np.max((maxstepsize, fit[j].stepsize))
    if hasattr(event[0].params, 'stepsize') == False or event[0].params.stepsize == None:
        for j in range(numevents):
            event[j].params.stepsize = maxstepsize
            #print(event[j].eventname, "new stepsize:", event[0].params.stepsize, file=printout)
    else:
        maxstepsize = event[0].params.stepsize
        for j in range(1,numevents):
            event[j].params.stepsize = maxstepsize
    print("MCMC stepsize:", maxstepsize, file=printout)
    ...
    for j in range(numevents):
        ...
        for i in range(cummodels[j],cummodels[j+1]):
            if   functype[i] == 'ortho':
                ...
                fit[j].meanfit   *= myfuncs[i](meanp  [iparams[i],0], funcx[i], fit[j].etc[k])
                                              ...
                                              or  functype[i] == 'sinusoidal'):
                # Average over high-resolution model
                hiresmodel = myfuncs[i](medianp[iparams[i],0], funcx[i], fit[j].etc[k])
                for n in range(len(fit[j].timebins)):
                    fit[j].medianfit[n] *= np.mean(hiresmodel[fit[j].timebins[n]])
                hiresmodel = myfuncs[i](meanp  [iparams[i],0], funcx[i], fit[j].etc[k])
                for n in range(len(fit[j].timebins)):
                    fit[j].meanfit[n]   *= np.mean(hiresmodel[fit[j].timebins[n]])
            else:
                fit[j].medianfit *= myfuncs[i](medianp[iparams[i],0], funcx[i], fit[j].etc[k])
                fit[j].meanfit   *= myfuncs[i](meanp  [iparams[i],0], funcx[i], fit[j].etc[k])
            k += 1
        fit[j].meanfit   *= fit[j].bestsys
        fit[j].medianfit *= fit[j].bestsys
        ...
        print("Reduced Chi^2: " + str(fit[j].redchisq), file=printout)
        print("Reduced Chi^2: " + str(fit[j].redchisq))
        print("Uncertainty Scaling factors: " + str(np.sqrt(fit[j].oldredchisq)), file=printout)
        ...
        fit[j].modelramp  = np.ones(fit[j].nmodelpts)
        ...
        for i in range(cummodels[j],cummodels[j+1]):
            ...
            #DO NOT REMOVE SINUSOIDAL FUNCTIONS FROM NORMALIZED RESULTS (FOR PHASE CURVES)
                ...
                if hasattr(fit[j], 'timebins'):
                    hiresmodel = myfuncs[i](rampp[iparams[i]], funcx[i], fit[j].etc[k])
                    for n in range(len(fit[j].timebins)):
                        fit[j].noecl[n] *= np.mean(hiresmodel[fit[j].timebins[n]])
                else:
                ...
                for n in range(len(fit[j].timebins)):
                    fit[j].noecl[n]  *= np.mean(hiresmodel[fit[j].timebins[n]])
                    fit[j].ramp [n]  *= np.mean(hiresmodel[fit[j].timebins[n]])
                hiresmodel = myfuncs[i](rampp[iparams[i]], funcxuc[i], fit[j].etc[k])
                for n in range(len(fit[j].timebinsuc)):
                    fit[j].rampuc[n] *= np.mean(hiresmodel[fit[j].timebinsuc[n]])
                fit[j].modelramp *= myfuncs[i](rampp[iparams[i]], fit[j].modelfuncx, fit[j].etc[k])
            elif functype[i] == 'posoffset':
                fit[j].noecl  *= myfuncs[i](rampp[iparams[i]], funcx  [i], fit[j].etc[k])
                fit[j].ramp   *= myfuncs[i](rampp[iparams[i]], funcx  [i], fit[j].etc[k])
                fit[j].rampuc *= myfuncs[i](rampp[iparams[i]], funcxuc[i], fit[j].etc[k])
                fit[j].modelramp *= myfuncs[i](rampp[iparams[i]], fit[j].posmodelfuncx, fit[j].etc[k])
            elif functype[i] == 'ippoly':
                fit[j].noecl  *= myfuncs[i](rampp[iparams[i]], funcx  [i], fit[j].etc[k])
                fit[j].ramp   *= myfuncs[i](rampp[iparams[i]], funcx  [i], fit[j].etc[k])
                fit[j].rampuc *= myfuncs[i](rampp[iparams[i]], funcxuc[i], fit[j].etc[k])
                fit[j].modelramp *= myfuncs[i](rampp[iparams[i]], fit[j].ipmodelfuncx, fit[j].etc[k])
            else:
                fit[j].noecl  *= myfuncs[i](rampp[iparams[i]], funcx  [i], fit[j].etc[k])
                fit[j].ramp   *= myfuncs[i](rampp[iparams[i]], funcx  [i], fit[j].etc[k])
                fit[j].rampuc *= myfuncs[i](rampp[iparams[i]], funcxuc[i], fit[j].etc[k])
                fit[j].modelramp *= myfuncs[i](rampp[iparams[i]], fit[j].modelfuncx, fit[j].etc[k])
            k += 1
        fit[j].noecl     *= fit[j].bestsys
        fit[j].ramp      *= fit[j].bestsys
        fit[j].rampuc    *= fit[j].bestsysuc
        #Try to multiply 'modelfit' by systematics
        fit[j].modelramp *= fit[j].systematics2
        #Error will only affect binned plot (fig6001)
        #print(fit[j].modelramp)
        '''
        try:
            fit[j].modelramp  *= fit[j].systematics2
        except ValueError as detail:
        ...
        fit[j].normmodelfit  = (fit[j].modelfit   / fit[j].modelramp).flatten()
        #print(fit[j].normmodelfit)
        ...
        for i in range(nbins[j]):
            ...
            fit[j].binresstd[i]    = np.std(fit[j].residuals[start:end]) / np.sqrt(fit[j].residuals.size)
    ...
    if hasattr(event[0].params, 'savedata') and event[0].params.savedata == False:
        ...
        for j in range(numevents):
            ...
            #pid.close()
            np.save(fit[j].allparamsfile, fit[j].allparams)
            ...
            if event[j].params.newortho == True:
                ...
                #pid.close()
                np.savez(fit[j].orthofile, invtrans=fit[j].invtrans, trans=fit[j].trans, origin=fit[j].origin, sigma=fit[j].orthosigma)
