...
static PyObject *mm_bilinint(PyObject *self, PyObject *args, PyObject *keywds);
...
static PyObject *mm_bilinint(PyObject *self, PyObject *args, PyObject *keywds)
{
  ...
  mastermapd  = (PyArrayObject *) PyList_GetItem(posflux,12);
  ...
  //printf("%lf, %lf, %lf \n", dovrmedian, maskmedian, meanbinflux);

 
  #pragma omp parallel for
  for(i=0;i<dims[0];i++)
    {
      IND(binflux,i) /= dovrmedian;
      IND(binstd, i) /= dovrmedian;
  	  // IND(mastermapFH, i) = IND(mastermapF, i) * (dovrmedian / maskmedian) / meanbinflux;
...
static PyMethodDef module_methods[] = {
  {"mm_bilinint",(PyCFunction)mm_bilinint,METH_VARARGS|METH_KEYWORDS,module_docstring},{NULL}};
...
PyMODINIT_FUNC
#if PY_MAJOR_VERSION >= 3
    PyInit_mm_bilinint(void)
#else
    initmm_bilinint(void)
#endif
{
	#if PY_MAJOR_VERSION >= 3
		PyObject *module;
		...
		return module;
	#else
	    PyObject *m = Py_InitModule3("mm_bilinint", module_methods, module_docstring);
		if (m == NULL)
			return;
		/* Load `numpy` functionality. */
		import_array();
