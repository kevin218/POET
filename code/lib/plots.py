...
def blissmap(event, fit, fignum, savefile=None, istitle=True, minnumpts=1, srcest=None):
    ...
    if 'mmbilinint' in fit.model:
        ...
        normMM = fit.mastermapFI*fit.mastermapovr/np.nanmean(fit.mastermapFI)
        plt.imshow(normMM.reshape(fit.xygrid[0].shape)[iymin:iymax+1,ixmin:ixmax+1], cmap=palette, vmin=vmin, vmax=vmax, origin='lower',
               extent=(xmin,xmax,ymin,ymax), aspect='auto', zorder=2)
